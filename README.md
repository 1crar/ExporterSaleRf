...
3. `getFile()` проверяет, содержится ли в поле `UF_CRM_1600310015` строка `миир рк`.
   Если содержится, скачивается разрешительный PDF в `./doc.pdf`.
...
6. Создается подключение к 1C через `win32com.client.Dispatch("V83.COMConnector")` (внутри цикла по сделкам, т.е. для каждой сделки отдельно).
...
9. Сделка переводится в стадию `C17:UC_BBPEXS` **до** вызова `operations1C()`. Если обработка завершится ошибкой, сделка останется на этой стадии (ручная обработка).
...
- документы 1C: заказ поставщику при необходимости, приобретение товаров и услуг, реализация товаров и услуг, входящий счет-фактура (`СчетФактураПолученный`), исходящий счет-фактура (`СчетФактураВыданный`), CMR;
...
- В `createPTU()` аргумент называется `RTUdate`, а внутри функции используется `RTUDate`. Фактически `NameError` не происходит, потому что `createPTU()` вызывается только из `operations1C()`, и Python подхватывает `RTUDate` из объемлющего scope через замыкание. Однако аргумент `RTUdate` остается неиспользованным.
